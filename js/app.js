...
(function () {
  ...
  /* ---------------- content chunks ----------------
     Markdown ships as one chunk per sidebar group (see build.js). BUNDLE is
     filled in as chunks arrive, so anything that reads it must go through
     withModule()/loadChunks() first. Chunks load via injected <script> tags,
     never fetch — that keeps the app working from a file:// path. */
  var MANIFEST = window.CONTENT_MANIFEST || { chunks: {}, chunkOf: {}, eager: [], version: '0' };
  var BUNDLE = {};
  var chunkState = {};                 // chunkId -> 'loading' | 'done' | 'error'
  var chunkWaiters = {};               // chunkId -> [callback]

  window.__contentChunk = function (id, mods) {
    for (var k in mods) if (Object.prototype.hasOwnProperty.call(mods, k)) BUNDLE[k] = mods[k];
    chunkState[id] = 'done';
    (chunkWaiters[id] || []).forEach(function (cb) { cb(null); });
    chunkWaiters[id] = [];
  };

  function loadChunk(id, cb) {
    if (!id || !Object.prototype.hasOwnProperty.call(MANIFEST.chunks, id)) { cb(new Error('unknown chunk ' + id)); return; }
    if (chunkState[id] === 'done') { cb(null); return; }
    (chunkWaiters[id] = chunkWaiters[id] || []).push(cb);
    if (chunkState[id] === 'loading') return;
    chunkState[id] = 'loading';
    var s = document.createElement('script');
    s.src = 'js/content/' + id + '.js?v=' + MANIFEST.chunks[id];
    s.onerror = function () {
      chunkState[id] = 'error';
      var ws = chunkWaiters[id] || []; chunkWaiters[id] = [];
      ws.forEach(function (w) { w(new Error('failed to load chunk ' + id)); });
    };
    document.head.appendChild(s);
  }

  /* Run cb(err, markdown) once the chunk owning `id` is in memory. */
  function withModule(id, cb) {
    if (Object.prototype.hasOwnProperty.call(BUNDLE, id)) { cb(null, BUNDLE[id]); return; }
    var chunk = MANIFEST.chunkOf[id];
    if (!chunk) { cb(null, undefined); return; }        // no such module at all
    loadChunk(chunk, function (err) { cb(err, BUNDLE[id]); });
  }

  /* Search needs every module, so it pays the full download once, on demand. */
  function loadAllChunks(cb) {
    var ids = Object.keys(MANIFEST.chunks).filter(function (c) { return chunkState[c] !== 'done'; });
    if (!ids.length) { cb(); return; }
    var left = ids.length;
    ids.forEach(function (c) { loadChunk(c, function () { if (--left === 0) cb(); }); });
  }
  ...
  function renderModule(id) {
    // Chunk already in memory → paint synchronously, no loading flash.
    if (Object.prototype.hasOwnProperty.call(BUNDLE, id)) { paintModule(id, BUNDLE[id]); return; }
    el.content.innerHTML = '<div class="content"><div class="empty"><h2>⏳ Lädt · Loading…</h2></div></div>';
    withModule(id, function (err, md) {
      if (currentRoute() !== id) return;          // reader already navigated away
      if (err) {
        el.content.innerHTML = '<div class="content"><div class="empty"><h2>⚠️ Ladefehler · Load error</h2>' +
          '<p>Der Inhalt für <code>' + esc(id) + '</code> konnte nicht geladen werden.</p>' +
          '<p style="max-width:520px;margin:10px auto">Check that <code>js/content/</code> exists and was generated by <code>node build.js</code>.</p>' +
  ...
  function paintModule(id, md) {
    var meta = itemById(id);
    if (!md) {
      el.content.innerHTML = '<div class="content"><div class="empty"><h2>📝 Modul wird geschrieben</h2>' +
        '<p>Dieses Modul (<code>' + esc(id) + '</code>) gibt es noch nicht.</p>' +
        '<p style="max-width:520px;margin:10px auto">If you generated content but it is missing here, run <code>node build.js</code> to rebuild the chunks in <code>js/content/</code>, then reload. Or type <b>“Continue”</b> in chat to have the next batch written.</p>' +
        '<p><a class="btn primary" href="#/home">← Home</a></p></div></div>';
      return;
    }
  ...
  function addTableSpeakers(root) {
    $all('table', root).forEach(function (tbl) {
      if (tbl.getAttribute('data-spk')) return;   // never inject twice
      tbl.setAttribute('data-spk', '1');
  ...
  function addSpoilers(root) {
    $all('.spoiler', root).forEach(function (sp) {
      var btn = $('.spoiler-btn', sp), body = $('.spoiler-body', sp);
      if (!btn || !body) return;
      btn.addEventListener('click', function () {
        var open = !body.hasAttribute('hidden');
        if (open) { body.setAttribute('hidden', ''); sp.removeAttribute('data-open'); }
  ...
  function renderHome() {
    ...
    // Keyed off the nav index, not BUNDLE — the target chunk may not be loaded.
    if (state.lastRoute && state.lastRoute !== 'home' && state.lastRoute.indexOf('search/') !== 0 && itemById(state.lastRoute)) {
      var li = itemById(state.lastRoute);
      html += '<p style="margin-top:18px"><a class="btn primary" href="#/' + state.lastRoute + '">▶ Weiterlernen · Continue: ' + esc(li.item.title) + '</a></p>';
    }
  ...
  function renderSearch(qRaw) {
    ...
    // Full-text search needs every chunk; download them once, then search.
    if (!searchIndex) {
      el.content.innerHTML = '<div class="content"><div class="empty"><h2>⏳ Suchindex wird aufgebaut…</h2>' +
        '<p>Building the search index — one moment.</p></div></div>';
      loadAllChunks(function () {
        buildSearchIndex();
        if (currentRoute() === 'search/' + encodeURIComponent(qRaw)) drawSearch(qRaw, q);
      });
      return;
    }
    drawSearch(qRaw, q);
  }
  function drawSearch(qRaw, q) {
    var res = [];
  ...
  function init() {
    ...
    el.themeBtn.addEventListener('click', function () { setTheme(document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark'); if (Object.prototype.hasOwnProperty.call(BUNDLE, currentRoute())) route(); });
    ...
    // Eager chunks (home + roadmap overview) must be in before the first paint.
    var eager = (MANIFEST.eager || []).slice();
    if (!eager.length) { route(); return; }
    var left = eager.length;
    eager.forEach(function (c) { loadChunk(c, function () { if (--left === 0) route(); }); });
  }
